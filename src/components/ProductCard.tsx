...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border/70 bg-card shadow-soft transition-shadow duration-300 hover:border-gold/50 hover:shadow-lift sm:rounded-3xl"
    >
      <div
        role="button"
        tabIndex={0}
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-3 hidden justify-center px-4 transition-all duration-300 md:flex md:translate-y-2 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100">
          <span className="flex items-center gap-1.5 rounded-full border border-gold/40 bg-background/90 px-3.5 py-1.5 text-xs font-medium text-gold-deep shadow-soft backdrop-blur-md transition-colors hover:bg-gold-deep hover:text-white">
      ...
      <button
        ...
        className={`absolute top-2 right-2 sm:top-3 sm:right-3 grid h-7 w-7 sm:h-9 sm:w-9 place-items-center rounded-full bg-card/90 backdrop-blur transition-all duration-300 md:opacity-0 md:group-hover:opacity-100 ${
          wished ? "text-destructive md:opacity-100" : "text-muted-foreground"
        }`}
      >
      ...
      <div className="flex flex-1 flex-col p-2.5 sm:p-4">
        ...
        <h3 className="mt-1 sm:mt-1.5 font-sans font-medium text-xs sm:text-base leading-snug text-foreground line-clamp-2 min-h-[2rem] sm:min-h-0">
          <button
            type="button"
            onClick={() => openQuickView(product)}
            className="text-left font-sans font-medium hover:text-gold-deep transition-colors line-clamp-2"
          >
            {product.name}
          </button>
        </h3>
        ...
        <div className="mt-3 sm:mt-4 flex items-center gap-1.5 sm:gap-2">
          <button
            ...
            className="flex flex-1 items-center justify-center gap-1 sm:gap-2 rounded-full bg-primary px-2 sm:px-4 py-2 sm:py-2.5 font-sans text-[0.68rem] sm:text-xs font-medium tracking-wide text-primary-foreground transition-all duration-300 hover:bg-brown active:scale-95 shadow-xs"
          >
          ...
          <button
            ...
            title="Quick View"
            className="grid h-8 w-8 sm:h-10 sm:w-10 shrink-0 place-items-center rounded-full border border-gold/40 text-gold-deep transition-all duration-200 hover:bg-gold-deep hover:text-white active:scale-95"
          >
